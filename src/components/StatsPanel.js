...
const StatsPanel = ({ isSidebarOpen, toggleSidebar, countryData }) => {
  return (
    <>
      <PanelContainer isOpen={isSidebarOpen}>
        ...
        {countryData ? (
          <>
            <h3>{countryData.attacks[0].sourceName} to {countryData.attacks[0].destinationName}</h3>
            <StatBox
              title="Attacks"
              items={countryData.attacks.map((attack, index) => ({
                name: `${attack.threatType} from ${attack.sourceName} to ${attack.destinationName}`,
                color: 'white' // Change color if needed
              }))}
            />
            <StatBox
              title="Attack Trends (Last 30 Days)"
              items={countryData.attackTrend.last30Days.map((count, index) => ({
                name: `Day ${index + 1}: ${count} attacks`,
                color: 'white' // Change color if needed
              }))}
            />
            <StatBox
              title="Malware Type Trends"
              items={Object.entries(countryData.attackTrend.malwareTypeTrends).map(([type, percentage]) => ({
